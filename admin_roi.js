...
(function($) {
    ...
    Drupal.behaviors.rubik.attach = function(context, settings) {
        ...
        if ( typeof settings != 'undefined' ){
            if ( typeof settings.rubik != 'undefined' ){
  ...
  Drupal.behaviors.admin_roi = {};
  Drupal.behaviors.admin_roi.attach = function(context, settings) {

    $('.node-form', context).once('admin-roi',function(){
        //Paragraphs to be opened by default.
        var prgphsToOpen = [];
        prgphsToOpen.push('.field-name-field-header');
        prgphsToOpen.push('.field-name-field-widget-options');

        var tasks = setInterval(function(){
            if(!prgphsToOpen.length){
                clearInterval(tasks);
            }
            else{
                var prghs = prgphsToOpen.shift();
                $(prghs, context).once(prghs.substring(1), function(){
                    $(':input.paragraphs-add-more-submit', $(this)).mousedown();
                });
            }
        }, 1000);
    });
